...
def health_check():
    """Simple liveness and status probe for the API and datastore."""
    try:
...
def generate_data():
    """Create a curated financial dataset via the integrated data curator."""
    try:
...
def download_csv(dataset_id):
    """Serve the selected dataset as a CSV file download."""
    try:
        if db is None:
...
def download_json(dataset_id):
    """Return the selected dataset as JSON for client consumption."""
    try:
        if db is None:
            return jsonify({'error': 'Database not available'}), 503
...
def get_analytics():
    """Summarize recent datasets and predictions with a basic accuracy snapshot."""
    try:
        if db is None:
            datasets = []
            predictions = []
...
def make_prediction():
    """Produce a lightweight next-step prediction from the latest data."""
    try:
        data = request.get_json()
...
def get_datasets():
    """List available datasets (returns an array; empty on error)."""
    try:
...
def debug_datasets():
    """Diagnostics for dataset retrieval and connection status."""
    try:
        debug_info = {
...
def debug_storage():
    """Diagnostics for metadata and historical price collections."""
    try:
...
def get_dataset(dataset_id):
    """Fetch a single dataset document by its identifier."""
    try:
        if db is None:
            return jsonify({'error': 'Database not available'}), 503
        dataset = db.get_dataset_by_id(dataset_id)
...
def get_prices():
    """Return historical OHLCV rows for charting (symbol required)."""
    try:
        if db is None:
            return jsonify({'error': 'Database not available'}), 503
...
def post_prediction():
    """Persist a forecast result document produced by a model run."""
    try:
        if db is None:
            return jsonify({'error': 'Database not available'}), 503
...
def list_predictions():
    """Filter prediction documents by symbol, horizon, and/or model."""
    try:
        if db is None:
            return jsonify([]), 200
        symbol = request.args.get('symbol')
...
def get_predictions_by_models():
    """Retrieve predictions for a symbol, optionally restricted to given models."""
    try:
        if db is None:
            return jsonify([]), 200
        symbol = request.args.get('symbol')
...
def upsert_metadata():
    """Create or update instrument metadata, including sources and logs."""
    try:
        if db is None:
            return jsonify({'error': 'Database not available'}), 503
        payload = request.get_json(force=True)
...
def get_metadata():
    """Return metadata for a single symbol or all, based on query."""
    try:
        if db is None:
            return jsonify([]), 200
        symbol = request.args.get('symbol')
...
def get_historical_public():
    """Public alias for historical prices: /get_historical?symbol=XYZ&limit=300"""
    try:
        if db is None:
            return jsonify({'error': 'Database not available'}), 503
        symbol = request.args.get('symbol')
...
def _parse_horizon_label_to_hours(h: str) -> int:
    try:
        s = (h or '').strip().lower()
        if s.endswith('h'):
...
def get_forecast_public():
    """Public preview of forecasts; supports models and horizon query params."""
    try:
        if db is None:
            return jsonify({'error': 'Database not available'}), 503
        symbol = request.args.get('symbol')
        ...
        preview_hours = _parse_horizon_label_to_hours(horizon_raw)
...
def run_forecast():
    """Execute chosen forecasting models on close prices and persist results."""
    try:
        if db is None:
            return jsonify({'error': 'Database not available'}), 503
        payload = request.get_json(force=True)
